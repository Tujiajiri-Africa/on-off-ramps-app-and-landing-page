...
import {addUserAddressToProfile, updateSession} from '@/actions/settings'
...
import {UserAddressProps} from '@/helpers/data'
import {fetchUserAddress} from '@/actions/settings'
...
export function UseAddressForm(){
    ...
    const handleUserAddressSubmission = (values: z.infer<typeof UserProfileAddressInfo>) => {
        ...
        startTransition(async() => {
            ...
            .then(async(data:any) => {
                ...
                if(data?.data.success){
                    ...
                    //await update({...userSessionData?.user, address: data?.data.address})
      ...
      const fetchddress = useCallback(async() => {
        const data =  await fetchUserAddress(userSessionData?.user.accessToken)
        if(data?.data?.error == null || data?.data?.error == undefined){
            console.error(data?.data?.data)
        }else{
            console.log(data?.data?.data)
            setUserAddress(data?.data?.data)
        }
      },[userSessionData,setUserAddress])
      
      const {error:queryError, status, data:addressData, isLoading, isError } = useQuery({
        queryKey: 'address',
        queryFn: fetchddress
    })
